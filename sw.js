const CACHE_NAME = 'imostar7-portfolio-v1.2.0';
const urlsToCache = [
  '/',
  '/index.html',
  '/manifest.json',
...
self.addEventListener('sync', event => {
  if (event.tag === 'background-sync') {
    ...
    // You can implement background sync for forms here
  }
});
...
self.addEventListener('push', event => {
  ...
  const options = {
    body: data.body || 'New update from IMOSTAR7',
  ...
  event.waitUntil(
    self.registration.showNotification(data.title || 'IMOSTAR7', options)
  );
});
